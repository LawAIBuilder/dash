    ...
    <div className="space-y-6 p-6">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <Button size="sm" onClick={() => { setNewPkgName(""); setNewPkgType("hearing_packet"); setCreateOpen(true); }}>
            ...
            packets.map((p) => (
              <div key={p.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border px-3 py-2">
                ...
                <Button variant="ghost" size="sm" asChild>
                  <Link to={`/cases/${caseId}/exhibits`}>Edit</Link>
                </Button>
              </div>
            ))
          )}
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-muted-foreground text-sm">Interrogatories, narratives, and other targets for discovery or petition flows.</p>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-muted-foreground text-sm">Runs the package worker (retrieval + draft + QA). Configure OPENAI_API_KEY on the API.</p>
        </CardHeader>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        exportPackageRunDocx(run.id)
                          .then(() => {
                            /* server returns path; alpha: copy path from toast in future */
                          })
                          .catch(() => {})
                      }
                    >
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              <label htmlFor="new-pkg-type" className="text-sm font-medium">Package type</label>
