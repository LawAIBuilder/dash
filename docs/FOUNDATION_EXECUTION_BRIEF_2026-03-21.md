...
- **Browser fallback behavior is now explicit:** when browser session auth is enabled on the API, the desktop shell expects a real login and does not silently fall back to browser bearer mode; when the API is intentionally running without `WC_SESSION_SECRET`, the shell now blocks with a clear misconfiguration message unless the desktop build explicitly enables `VITE_WC_API_KEY` plus `VITE_WC_ENABLE_API_KEY_FALLBACK` ([`AppShell.tsx`](../apps/desktop/src/layouts/AppShell.tsx), [`config.ts`](../apps/desktop/src/config.ts)).
...
| **D** | Remove **`VITE_WC_API_KEY`** from normal hosted usage; retain server **`WC_API_KEY`** only for M2M or break-glass if absolutely required. **Implemented for the normal hosted browser path**: browser bearer mode is now explicit opt-in instead of implicit whenever `VITE_WC_API_KEY` is present, and the desktop shell now blocks obvious API-key-only misconfiguration instead of making silent unauthenticated requests. |

---

## Blueprint canonicalization

### Current foundation now on `main`

- **Schema exists now:** `blueprints`, `blueprint_versions`, and nullable `blueprint_version_id` bindings on `exhibit_packets` and `package_runs` are now in the authoritative migration chain ([`schema.ts`](../apps/api/src/schema.ts)).
- **Seeded runtime scope exists now:** the first three package types now seed active blueprint versions on foundation seed: `hearing_packet`, `claim_petition`, and `discovery_response` ([`seed.ts`](../apps/api/src/seed.ts), [`blueprints.ts`](../apps/api/src/blueprints.ts)).
- **Runtime binding exists now:** new exhibit packets bind an active blueprint version by package type, package runs persist `blueprint_version_id`, and `runPackageWorker` now resolves prompt text, default model, and retrieval-profile flags from the bound blueprint version ([`exhibits.ts`](../apps/api/src/exhibits.ts), [`ai-service.ts`](../apps/api/src/ai-service.ts), [`blueprints.ts`](../apps/api/src/blueprints.ts), [`retrieval.ts`](../apps/api/src/retrieval.ts)).
- **Inspection surface exists now:** packet and run payloads now expose blueprint name/version metadata for operator-visible product surfaces ([`exhibits.ts`](../apps/api/src/exhibits.ts), [`package-workbench-routes.ts`](../apps/api/src/routes/package-workbench-routes.ts), [`CasePackagesPage.tsx`](../apps/desktop/src/pages/cases/CasePackagesPage.tsx)).
...
| **3** | Blueprints: `blueprints` + `blueprint_versions`, wire three package types, migrate prompts/retrieval references incrementally. **Started on `main`** with seeded blueprint versions, packet/run binding, and packet/run UI metadata. |
