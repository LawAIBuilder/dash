...
- **Case access is partially real now:** package/workbench and case-data routes now require a **`case_memberships`** row for non-admin session users, while admins still bypass and API-key mode remains an explicit transitional bypass ([`auth.ts`](../apps/api/src/auth.ts), [`package-workbench-routes.ts`](../apps/api/src/routes/package-workbench-routes.ts), [`case-data-routes.ts`](../apps/api/src/routes/case-data-routes.ts), [`schema.ts`](../apps/api/src/schema.ts)).
...
| **C** | Route guards: case read/write, package run/approve, connector manage, ops/admin. **Partially implemented**: package/workbench and case-data routes now enforce `case_memberships` for non-admin session users with admin override and API-key transitional bypass; connector routes now split between admin-only tenant-level actions and case-membership-gated per-case sync/hydrate actions; admins now have a real membership-management/backfill path for legacy matters. |
