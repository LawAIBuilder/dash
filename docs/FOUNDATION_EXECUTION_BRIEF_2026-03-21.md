...
| **C** | Route guards: case read/write, package run/approve, connector manage, ops/admin. **Partially implemented**: package/workbench and case-data routes now enforce `case_memberships` for non-admin session users with admin override and API-key transitional bypass, and admins now have a real membership-management/backfill path for legacy matters. |
